...
void Titi_Matrix::init(byte _data, byte _load, byte _clock, byte _num, int _rotation)
{
  data = _data;
  load = _load;
  clock = _clock;
...
void Titi_Matrix::setIntensity(byte intensity)
{
	setCommand(max7219_reg_intensity, intensity);
}

void Titi_Matrix::clearMatrix()
{
	for (int i=0; i<8; i++) 
		setColumnAll(i,0);
...
void Titi_Matrix::setCommand(byte command, byte value)
{
#if defined(ESP32)
...
void Titi_Matrix::setColumn(byte col, byte value)
{
	int n = col / 8;
	int c = col % 8;
#if defined(ESP32)
...
void Titi_Matrix::setColumnAll(byte col, byte value)
{
#if defined(ESP32)
    for (int i=0; i<num; i++) 
    {
...
void Titi_Matrix::setDot(byte col, byte row, byte value)
{
...
// routine for Titi and ZOWI, for the 6 x 5 matrix
void Titi_Matrix::writeFull(unsigned long value) {
  if (rotation == 1) {
	for (int r=0; r<5;r++){
            for (int c=0; c<6; c++){
...
void Titi_Matrix::sendChar (const byte data, byte pos, byte number, byte scrollspeed){
